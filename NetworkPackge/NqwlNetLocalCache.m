...
static NSString *  NqwlCacheFileProcessingQueue = @"com.nqwl.NetCache";
static NSString *  NqwlCacheDocument = @"NqwlCache";
static const NSInteger NqwlDefaultCacheMaxDeadline = 60 * 60 * 24 * 30; // 1 个月

@interface NqwlNetLocalCache (){
    NSCache          *_memoryCache;//cache 用来解决频繁从文件系统中 读取文件的 系能问题
    NSString         *_cachePath;
    NSFileManager    *_fileManager;
    dispatch_queue_t _NqwlIOQueue;
    NSMutableSet     *_protectCaches;
}

@end

@implementation NqwlNetLocalCache


+ (instancetype)sharedInstance
{
    static NqwlNetLocalCache *instance;
...
- (instancetype)init
{
    ...
    if (self) {
        ...
        _NqwlIOQueue = dispatch_queue_create([NqwlCacheFileProcessingQueue UTF8String], DISPATCH_QUEUE_CONCURRENT);
        _maxCacheDeadline = NqwlDefaultCacheMaxDeadline;
        //build setting -> implicit retain of 'self'
        dispatch_sync(_NqwlIOQueue, ^{
            _fileManager = [NSFileManager new];
            NSArray *paths = NSSearchPathForDirectoriesInDomains(NSCachesDirectory, NSUserDomainMask, YES);
            _cachePath = [[paths objectAtIndex:0] stringByAppendingPathComponent:NqwlCacheDocument];
            BOOL isDir;
...
- (id)searchCacheWithUrl:(NSString *)urlkey{
    ...
    object = [self decodeData:object];
    return object;
}
- (id)decodeData:(id)object {
    return object;
}
/**
 保存缓存
...
- (void)saveCacheData:(id<NSCopying>)data forKey:(NSString*)key{
    ...
    data = [self encryData:data];
    [_memoryCache setObject:data forKey:key];
    dispatch_async(_NqwlIOQueue, ^{
